...
select 고객회사명, 전화번호, 
	lpad (right (고객회사명,char_length(고객회사명)-2),char_length(고객회사명) ,'*') "고객회사명2",
-- concat('**', substr(고객회사명, 3)) as 고객회사명2
	replace (replace(전화번호, ")", "-"),"(","") "전화번호2"
...
-- 실주문금액 : 주문금액 - 할인금액
select 주문수량*단가 "주문금액", 
round (주문수량*단가*할인율)"할인금액", 
round ((주문수량*단가)-(주문수량*단가*할인율)) "실주문금액"
-- truncate () 사용 권고
from 주문세부;
...
select 이름, 생일, 입사일,
	...
	datediff(now(),입사일) "입사일수",
	adddate(입사일, interval 500 day) "입사500일후" 
from 사원;
...
select 담당자명, 고객회사명, 도시, 
	if (도시 like '%특별%' or 도시 like '%광역%', "대도시","도시") "도시구분",
	마일리지,
	case when 마일리지 > 100000 then "VVIP"
	when 마일리지 > 10000 then "VIP"
	else "일반고객" end "마일리지구분"
from 고객;
