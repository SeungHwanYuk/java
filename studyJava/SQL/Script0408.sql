/*  SQL (Strucktured Query Langauge) : 구조화된 요청 언어. Database에서 필요한 기능을 실행시키기 위한 명령어 모음
...
/*테이블내의 컬럼이외에 새로운 컬럼을 보여줄 수 있다. (실제 데이터베이스에는 저장안됨)*/
...
where 도시 = '서울특별시'
order by 마일리지 desc;
/*역시 키워드 사용 순서가 중요함. oder by가 제일 뒤
oder by는 디폴트가 오름차순이므로 내림차순은 desc를 사용해야함
'서울특별시'처럼 단따옴표가 기본. 쌍따옴표는 대소문자가 구별되어야하는 경우에만 사용하는것이 권장됨*/
...
order by 마일리지 desc
limit 10, 10;
/*내림차순으로 상위 10명을 제외하고 그 이후 10명의 데이터를 읽는 방법임.*/

/*고객들이 위치한 도시정보만 보고싶은 경우*/
select 도시 from 고객;
